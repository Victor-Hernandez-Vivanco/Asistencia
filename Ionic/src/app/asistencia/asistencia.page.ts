...
export class AsistenciaPage implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log('usuario nombre del ngoninit', this.usuario?.idSeccion);

    this.fechaActual = this.obtenerFechaActual();
    this.horaActual = this.obtenerHoraActual();

    this.apiService.getAllBySeccionId(this.usuario.idSeccion).subscribe(
      (data: any) => {
        console.log('data', data);
        this.alumnos = data.filter((item: any) => item.seccion.idseccion === this.usuario.idSeccion);
