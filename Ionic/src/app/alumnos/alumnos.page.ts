...
export class AlumnosPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.apiService.getAlumnoSeccionData().subscribe((data: any) => {
      const items = data.map((el: any) => keysToUpperCase(el));
      this.alumnosSeccion = items;
    });
  }
    ...
    try {
      ...
      element.onDidDismiss().then((result) => {
        ...
        if (barcode) {
          ...
          // get value of first barcode
          this.displayValue = barcode.displayValue.split('_')[1];

          try {
            const seccionId = Number(this.displayValue);
            const found = this.alumnosSeccion.find(
              (el) => el.IDSECCION === seccionId && el.IDALUMNO === this.user.idAlumno
            );

            if (!found) {
              this.msgOne = '❌ Usted no está inscrito en esta sección';
              this.msgTwo = 'Por favor, intente nuevamente';
              return;
            }
